...
public class MoodDatabaseService extends BaseDatabaseService {
    ...
    private MoodDatabaseService(String username) {
        super();
        this.username = username;
        moodsRef = db.collection("moods");
    }

    public static MoodDatabaseService getInstance(String username) {
        if (instance == null || !instance.username.equals(username)) {
            instance = new MoodDatabaseService(username);
        }
        return instance;
    }
    ...
    // Method to get all the documents in the moods collection for the current user
